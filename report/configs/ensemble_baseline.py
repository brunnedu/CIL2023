...
TRAIN_CONFIG = {
    'experiment_id': 'ensemble_baselines',  # should be changed for every run
    'resume_from_checkpoint': False,  # set full experiment id (including timestamp) to resume from checkpoint
    'train_dataset_kwargs': {
        'pred_dirs': [f"out/{exp_id}/data5k" for exp_id in ENSEMBLE_EXPERIMENT_IDS],
...
RUN_CONFIG = {
    'experiment_id': 'ensemble_baselines',
    'dataset_kwargs': {
        'pred_dirs': [f"out/{exp_id}/test" for exp_id in ENSEMBLE_EXPERIMENT_IDS],
        'data_dir': 'data/test',
